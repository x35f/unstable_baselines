...
class GaussianPolicyNetwork(BasePolicyNetwork):
    def __init__(self, 
        ...
        # set scaler
        if action_space is None:
            self.action_scale = torch.tensor(1., dtype=torch.float, device=util.device)
            self.action_bias = torch.tensor(0., dtype=torch.float, device=util.device)
        else:
            self.action_scale = torch.tensor((action_space.high-action_space.low)/2.0, dtype=torch.float, device=util.device)
            self.action_bias = torch.tensor((action_space.high+action_space.low)/2.0, dtype=torch.float, device=util.device)
        ...
        self.log_std_min =torch.tensor(log_std_min, dtype=torch.float, device=util.device )
        self.log_std_max = torch.tensor(log_std_max, dtype=torch.float, device=util.device )
